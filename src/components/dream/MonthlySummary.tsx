...
export function MonthlySummary({ entries, courseId, label }: MonthlySummaryProps) {
  ...
  const mult10 = Math.round(multiplierForYears(course.annualRate, 10) * 10) / 10;
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <p className="text-[10px] text-muted-foreground mt-2">
          ※{course.indexName}（年利{(course.annualRate * 100).toFixed(0)}%）で複利計算：10年後{mult10}倍・20年後{mult20}倍換算
        </p>
      </CardContent>
    </Card>
  );
